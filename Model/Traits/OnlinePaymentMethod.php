...
trait OnlinePaymentMethod
{
    ...
    protected function processReferenceTransaction(
        ...
        $onepey = "\\OnePEY\\$transactionType";
        $onepey = new $onepey;

        $onepey->setParentUid($data['reference_id']);
        $onepey->money->setAmount($data['amount']);
        $onepey->money->setCurrency($data['currency']);

        if (strtolower($transactionType) == $helper::REFUND)
          $onepey->setReason($data['reason']);

        $responseObject = $onepey->submit();
