...
namespace OnePEY\OnePEY\Model\Method;

/**
 * Checkout Payment Method Model Class
 * Class Checkout
 * @package OnePEY\OnePEY\Model\Method
 */
class Checkout extends \Magento\Payment\Model\Method\AbstractMethod
{
    use \OnePEY\OnePEY\Model\Traits\OnlinePaymentMethod;

    const CODE = 'onepey_checkout';
    /**
...
//     }
    ...
    protected function checkout($data)
    {
      ...
      //return new \OnePEY\Response(json_encode(array()));
    ...
    public function order(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
    	...
    	if ($transactionTypes != null && isset($transactionTypes[0]) 
      		&& $transactionTypes[0] == \OnePEY\OnePEY\Helper\Data::AUTHORIZE){
        ...
        try {
            ...
            $responseObject = $this->checkout($data); /* @var $responseObject \OnePEY\Response */
            
            $isOnePEYSuccessful = $responseObject->isValid() && !empty($responseObject->getRedirectUrl());
            
            if (!$isOnePEYSuccessful) {
                $errorMessage = $responseObject->getMessage();

                $this->getCheckoutSession()->setOnePEYLastCheckoutError(
                    $errorMessage
                );
            ...
            $customerRedirect = new \OnePEY\CustomerRedirectHostedPage($responseObject->getRedirectUrl(), $responseObject->getUid());
            $customerRedirect->money =  new \OnePEY\Money($data['order']['amount'], $data['order']['currency']);
            ...
            $this->getCheckoutSession()->setOnePEYCheckoutRedirectUrl(
