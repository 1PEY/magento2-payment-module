...
class Direct extends \Magento\Payment\Model\Method\Cc
{
    ...
    protected function getTransactionTypeRequestClassName($transactionType)
    {
        ...
        return "\\OnePEY\\{$requestClassName}";
    }
     ...
     * @return $this
     * @throws \Exception
     * @throws \OnePEY\Exceptions\ErrorAPI
     */
    protected function processTransaction(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $transaction = new $this->getTransactionTypeRequestClassName(
          $transactionType
        ); /* @var $transaction \OnePEY\AuthorizationOperation */
        ...
        try {
            $response = $transaction->submit(); /* @var $responseObject \OnePEY\Response */
        ...
        $this->setOnePEYResponse(
            $response
        );

        $onepey_response = $this->getModuleHelper()->getArrayFromGatewayResponse(
            $this->getOnePEYResponse()->getResponse()
        );
        ...
        $isTransactionFailed = $response->isFailed() || $response->isDeclined() || $response->isError() || !$response->isValid();
            !$response->isSuccess() && !$this->getOnePEYResponse()->isIncomplete();

        if ($isTransactionFailed) {
            $errorMessage = $this->getModuleHelper()->getErrorMessageFromGatewayResponse(
                $this->getOnePEYResponse()
            );
    ...
    public function setRedirectUrl($redirectUrl)
    {
        ...
        $this->getCheckoutSession()->setOnePEYCheckoutRedirectUrl($redirectUrl);
    }
